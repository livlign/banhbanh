...
namespace SubSonic
{
    public class Scaffold : Control, INamingContainer
    {
        ...
        private bool _useEmbeddedStyles;
        private bool _autoGenerateM2M = true;
        private bool _returnOnSave;
        ...
        public string PrimaryKeyControlValue
        {
            get
            {
                if (TableSchema.PrimaryKey.AutoIncrement || (TableSchema.PrimaryKey.DataType == DbType.Guid))
                    return ((Label)FindControl(PK_ID)).Text;
                return ((TextBox)FindControl(PK_ID)).Text;
            }
        }
        ...
        [Category("Data")]
        [DefaultValue(true)]
        public bool ReturnOnSave
        {
            get { return _returnOnSave; }
            set { _returnOnSave = value; }
        }
        ...
        private TableSchema.ManyToManyRelationshipCollection ManyToManyCollection
        {
            get
            {
                ...
                {
                    string[] m2mTables = ManyToManyMap.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        {
            get { return _useEmbeddedStyles; }
            set { _useEmbeddedStyles = value; }
        }
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            if(UseEmbeddedStyles)
            {
                ...
                grid.BorderColor = System.Drawing.Color.FromArgb(220, 220, 220);
                grid.FooterStyle.BackColor = System.Drawing.Color.FromArgb(204, 204, 204);
                grid.RowStyle.BackColor = System.Drawing.Color.FromArgb(238, 238, 238);
                grid.AlternatingRowStyle.BackColor = System.Drawing.Color.FromArgb(255, 255, 255);
                grid.PagerStyle.BackColor = System.Drawing.Color.FromArgb(153, 153, 153);
                grid.HeaderStyle.BackColor = System.Drawing.Color.FromArgb(220, 220, 220);
        ...
        protected override void CreateChildControls()
        {
            ...
            {
                lblTitle.Text = "No <strong>TableName</strong> was specified. Please set this property, or set <strong>ScaffoldType</strong> to 'Auto'.";
                return;
            }
            ...
            {
                if(activeTableSchema == null)
                    lblTitle.Text = "No table was located with the <strong>TableName</strong> specified. Please change the property, or set <strong>ScaffoldType</strong> to 'Auto'.";
                else
                    lblTitle.Text = "The specified table does not contain a primary key, a requirement for use of the scaffolds, and much of SubSonic.";
                return;
            }
            ...
            {
                Panel pnlNavigator = new Panel();
                pnlNavigator.ID = "pnlNavigator";
            ...
            foreach (TableSchema.TableColumn col in TableSchema.Columns)
            {
                if (col.DataType != DbType.Binary && Utility.IsWritableColumn(col))
                {
                    ...
                    if (ctrl != null)
                    {
                        ...
                        if (oVal != null && col.DataType == DbType.Boolean)
                        {
                        ...
                        else if (oVal != null && col.DataType == DbType.DateTime)
                        {
        ...
        protected void AddRow(HtmlTable tbl, string cellValue1, Control control)
        {
            ...
            td.InnerHtml = "<span style=\"font-weight:bold\">" + cellValue1 + "</span>";
        ...
        protected void ShowMessage(string message)
        {
            ...
            if (lblMessage != null)
                lblMessage.Text = message + "<br/><span style=\"font-style:italic\">" + TableSchema.Provider.Now + "</span>";
        }
        ...
        /// <returns></returns>
        protected Control GetEditControl(TableSchema.TableColumn col)
        {
            WebControl cOut = null;
            string colName = col.ColumnName.ToLower();
            //use special care with the Primary Key
            if (col.IsPrimaryKey && FindControl(PK_ID) == null)
            {
            ...
            else
            {
                ...
                if (col.IsForeignKey)
                {
                    ...
                    string fkTableName = col.ForeignKeyTableName;
                    ...
                    if(!String.IsNullOrEmpty(fkTableName))
                    {
                        ...
                        bool isSortable = Utility.GetEffectiveMaxLength(tbl.Columns[1]) < 250;
                        ...
                        SqlQuery qry = new Select(tbl.Provider, tbl.Columns[0].ColumnName, tbl.Columns[1].ColumnName).From(tbl);

                        if (isSortable)
                            qry.OrderAsc(tbl.Columns[1].ColumnName);
        ...
        protected void BindEditor(TableSchema.Table tbl, string keyID)
        {
            ...
            using (IDataReader rdr = qry.ExecuteReader())
            {
                if(rdr.Read())
                {
                    foreach(TableSchema.TableColumn col in tbl.Columns)
                    {
                        if(col.IsPrimaryKey)
                        {
                            Control ctrl = FindControl(PK_ID);
                            if(ctrl != null)
                            {
        ...
        protected void BindGrid(string orderBy)
        {
            ...
            {
                Query query = new Query(TableSchema);

                if (!String.IsNullOrEmpty(_whereExpression))
                    query.WHERE(_whereExpression);

                if (_whereCollection != null)
                    query.wheres = _whereCollection;
                ...
                grid.DataKeyNames = new string[] { dataKey };
                ...
                for (int i = 0; i < TableSchema.Columns.Count; i++)
                {
                    ...
                    field.DataField = dt.Columns[dtColIndex].ColumnName;
                    field.SortExpression = dt.Columns[dtColIndex].ColumnName;
                    ...
                    if (col.IsForeignKey)
                    {
                        TableSchema.Table fkSchema = col.ForeignKeyTableName == null ? DataService.GetForeignKeyTable(col, TableSchema) : DataService.GetSchema(col.ForeignKeyTableName, ProviderName, TableType.Table);
        ...
        protected void CreateEditor(Control destination, bool isEdit)
        {
            ...
            else
            {
                ...
                if (ShowScaffoldCaption)
                    AddRow(tbl, "<h2>" + TableSchema.DisplayName + " Editor</h2>", 2);
                    ...
                    if (ctrl != null)
                    {
                        ...
                        if (ctrl.GetType() == typeof(TextBox))
                        {
                            ...
                            if (tbx.TextMode == TextBoxMode.MultiLine)
                            {
                                ...
                                string jsAttribute = String.Concat("return imposeMaxLength(event, this, ", maxLength, ", ", tbl.Rows.Count, ");");
                                tbx.Attributes.Add("onkeyup", jsAttribute);
                                tbx.Attributes.Add("onChange", jsAttribute);
                                LiteralControl lc =
                                    new LiteralControl(String.Concat("<div style='padding: 2px;'><div style='float:left'>Characters Remaining:&nbsp;</div><div id=\"counter", tbl.Rows.Count,
                                                       "\" style=\"visibility:hidden\">",  remainingLength, "</div></div>"));
        ...
        private HtmlTable CreateManyMapper()
        {
            ...
            if (ShowScaffoldCaption)
                AddRow(tbl, String.Concat("<h2>", TableSchema.DisplayName, " Map</h2>"), 2);
        ...
        /// </summary>
        /// <param name="tbl">The TBL.</param>
        private void AddManyToMany(HtmlTable tbl)
        {
            foreach (TableSchema.ManyToManyRelationship m2m in ManyToManyCollection)
            {
                TableSchema.Table mapTable = DataService.GetSchema(m2m.MapTableName, ProviderName);
                ...
                chk.ID = mapTable.ClassName;
                AddRow(tbl, mapTable.DisplayName, chk);
                chk.Items.Clear();
                chk.RepeatColumns = 2;

                bool isSortable = Utility.GetEffectiveMaxLength(foreignTable.Columns[1]) < 250;
        ...
        {
            PrimaryKeyValue = grid.DataKeys[e.NewEditIndex].Value.ToString();
